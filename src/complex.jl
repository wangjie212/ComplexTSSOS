...
include("E:\\Programs\\blockpop\\TSSOS\\src\\chordal_extension.jl")
...
function cs_mtssos(supp::Vector{Vector{Vector{Vector{UInt16}}}},coe,n,d,dg;numeq=0,foc=100,CS="MF",minimize=true,assign="first",TS="block",QUIET=false,solve=true,solution=false,MomentOne=false)
    m=length(supp)-1
    ...
    opt,supp0=blockcpop_mix(n,m,dg,rlorder,supp,coe,fbasis,gbasis,cliques,cql,cliquesize,J,ncc,blocks,cl,blocksize,numeq=numeq,QUIET=QUIET,TS=TS,solve=solve,solution=solution,MomentOne=MomentOne)
...
function cs_mtssos_higher!(data;TS="block",QUIET=false,solve=true,solution=false,MomentOne=true)
    n=data.n
    m=data.m
    dg=data.dg
    supp=data.supp
    ...
    if status==1
        opt,supp0=blockcpop_mix(n,m,dg,rlorder,supp,coe,fbasis,gbasis,cliques,cql,cliquesize,J,ncc,blocks,cl,blocksize,numeq=numeq,QUIET=QUIET,solve=solve,solution=solution,MomentOne=MomentOne)
        data.supp0=supp0
        data.blocks=blocks
        data.cl=cl
...
function blockcpop_mix(n,m,dg,rlorder,supp,coe,fbasis,gbasis,cliques,cql,cliquesize,J,ncc,blocks,cl,blocksize;numeq=0,QUIET=false,TS="block",solve=true,solution=false,MomentOne=false)
    ...
    if solve==true
        ...
        for i=1:cql
            if (MomentOne==true||solution==true)&&TS!=false&&rlorder[i]>1
                bs=cliquesize[i]+1
...
function get_graph(tsupp,basis)
    lb=length(basis)
    G=SimpleGraph(lb)
    ltsupp=length(tsupp)
    for i = 1:lb, j = i+1:lb
...
function get_cgraph(tsupp,supp,lt,basis)
    lb=length(basis)
    ltsupp=length(tsupp)
    G=SimpleGraph(lb)
    for i = 1:lb, j = i+1:lb
...
function get_cblocks!(m,tsupp,gsupp,lt,fbasis,gbasis;gblocks=[],gcl=[],gblocksize=[],ub=[],sizes=[],TS="block",minimize=false,QUIET=true,merge=false)
    if isempty(gblocks)
...
function clique_decomp(n,m,dg,supp;order="min",alg="MD",minimize=false)
    if alg==false
